# Response to LogicArt Replit Team
...
**Decision:** The `@logicart/bridge` package **MUST** own the parsing logic (Code → Flowchart Nodes).
...
### 4. Timeline & Delivery
*   **Delivery Format:** We will provide `@logicart/bridge` as a **local package** (source code) first. This allows for rapid iteration without waiting for npm publishes.
...
**Next Step:** We are finalizing the extraction of the bridge code. We will push the `@logicart/bridge` folder to the repository shortly.
